...
namespace UniTimetable
{
    ...
    class UQSiNetImporter : Importer
    {
            ...
            : base()
        {
            ...
            CreatedBy_ = "Jack Valmadre (updated by Jeremy Herbert, Declan Vong)";
            LastUpdated_ = "July 2015";
        ...
        protected override Timetable Parse()
        {
            ...
            Regex timetable_parser = new Regex(
                ...
                @"<td>(?<weeks_taught>.*?)</td>" +          // time period over which the stream runs
                @"<td>(?<signon_avail>.*?)</td>" +          // whether the signon is available yet (pointless for us)
                @"<td>(?<student_signon>.*?)</td>" +         // status of the signon
                @"<td>(?<group_name>.*?)</td>" +            // unknown, seems to be the same as session_type_name for the most part
                @"</tr>"                                    // close row
            );
